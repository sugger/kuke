...
class sheng_fphone extends Think {
    /**
     * 卡号：15487011706410800 
        卡密：433452311493833036 
    */
    public function BeginPay($arr,$partner=null) {
        ...
        $new_data = $shengpay->parse_card_data($data);
        return $new_data;
    }
    ...
    public function Sharing($data,$partner)
    {
      ...
      if(!empty($order)&&$order['order_status']!="0,0,0"&&!empty($partner['bank_account'])){
        $obj = new Api();
        $frequest=$obj->parse_fRequest(array_merge($order,$data) ,$partner,0);
