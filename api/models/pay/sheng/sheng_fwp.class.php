...
class sheng_fwp extends Think
{
	...
	public function BeginPay($arr,$partner=null) {
		...
		$p_request=$obj->parse_pRequest($c_return,array('paytype'=>'wp')) ;
    ...
    public function create_order($arr)
    {
		...
		$c_request=$obj->parse_cRequest($request) ;//获取收单数据
		$c_return=$obj->send($c_request);//发送收单数据
		S( 'request' .$arr['orderid'] ,$request, $expire );
    ...
    {	
    	...
    	if($cache){
			...
			$obj= new Api();
			$p_request=$obj->parse_pRequest($this->pay,array('paytype'=>'wp'));
			$p_return=$obj->send($p_request);
			$data['returnInfo']=get_object_vars($p_return['return']->returnInfo) ;
			$data['extension'] =get_object_vars($p_return['return']->extension) ;
			S('p_' . $order,$data,300);
         ...
         if ($data['returnInfo']->errorCode == 'S0533001') {
         	F('apErrS0533001',$this->pay);
         	F('acErrS0533001',S('request' .$order) );
         }
        die(json_encode($data)) ;
    }
  }
